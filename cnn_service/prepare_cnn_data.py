...
from tensorflow.keras.models import load_model  # necessário para carregar depois
...
X = np.array(imagens_saudavel + imagens_doente, dtype=np.float32) / 255.0
...
print(f"Loss teste: {loss:.4f}, Acurácia teste: {acc:.4f}")

model.save("cnn_agro_model.h5")
print("Modelo salvo como cnn_agro_model.h5")

def prever_imagem(caminho):
    img = Image.open(caminho).convert('RGB').resize(IMG_SIZE)
    img_array = np.array(img, dtype=np.float32) / 255.0
    img_array = np.expand_dims(img_array, axis=0)
    pred = model.predict(img_array)[0][0]
    return "Saudável" if pred >= 0.5 else "Doente"
